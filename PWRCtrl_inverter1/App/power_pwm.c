...
void pwm_config()
{
	...
	timere_initpara.alignedmode = TIMER_COUNTER_EDGE;      	// 中央对齐                
	timere_initpara.counterdirection = TIMER_COUNTER_UP;    // 向上计数    	                    
	timere_initpara.period = 2399;      									  // 周期 
...
//	timer_channel_output_mode_config(TIMER1,TIMER_CH_1,TIMER_OC_MODE_PWM1);				// 配置定时器通道输出比较模式
//	timer_channel_output_shadow_config(TIMER1,TIMER_CH_1,TIMER_OC_SHADOW_DISABLE);// 配置定时器通道输出影子寄存器
	...
	timere_initpara.counterdirection = TIMER_COUNTER_UP;    // 向上计数    	                    
	timere_initpara.period = 7;      									  // 周期    T = 10000 * 1MHZ = 10ms  f = 100HZ   
	timere_initpara.clockdivision = TIMER_CKDIV_DIV1;    	  // 分频因子
...
void timer3_pwm_config() 
{
	...
	timere_initpara.alignedmode = TIMER_COUNTER_EDGE;      	// 边缘对齐                
	timere_initpara.counterdirection = TIMER_COUNTER_UP;    // 向上计数    	                    
	timere_initpara.period = 2399;      									  // 周期    T = 10000 * 1MHZ = 10ms  f = 100HZ   
	timere_initpara.clockdivision = TIMER_CKDIV_DIV1;    	  // 分频因子
	//timere_initpara.repetitioncounter = 0;                  // 重复计数器 0-255  
	...
	timer_channel_output_mode_config(TIMER3,TIMER_CH_2,TIMER_OC_MODE_PWM0);	// 配置定时器通道输出比较模式
	...
	timer_channel_output_mode_config(TIMER3,TIMER_CH_3,TIMER_OC_MODE_PWM0);				// 配置定时器通道输出比较模式
